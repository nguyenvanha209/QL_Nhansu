...
export default function BaoCaoPage() {
  ...
  const enriched = useMemo(() => filtered.map((vc) => {
    ...
    return { ...vc, donViTen: dv?.ten ?? '', chucDanhTen: cd?.ten ?? '', bac: hsl?.bac ?? 0, heSo: hsl?.heSo ?? 0, luongChinh: hsl ? getLuongChinh(hsl.heSo) : 0, ngayNangTiep: hsl?.ngayNangLuongTiepTheo ?? '' }
  ...
  const vcCols = [
    ...
    { title: 'Lương chính', dataIndex: 'luongChinh', key: 'lc', render: (v: number) => formatVND(v) },
  ...
  const exportVCPdf = () => {
    exportToPdf(
      ...
      ['Mã VC', 'Họ tên', 'Đơn vị', 'Chức danh', 'Bậc', 'Hệ số', 'Lương chính (VNĐ)'],
      enriched.map((r) => [r.ma, `${r.ho} ${r.ten}`, r.donViTen, r.chucDanhTen, r.bac, r.heSo, r.luongChinh.toLocaleString()]),
      'BaoCao_VienChuc'
    )
  }
